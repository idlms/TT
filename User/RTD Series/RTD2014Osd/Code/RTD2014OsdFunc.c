...
void OsdFuncClearOsdAndXOffset(BYTE ucRow, BYTE ucCol, BYTE ucXOffset, BYTE ucWidth, BYTE ucHeight);	
...
#endif
void LCDtest_ShowBurninPicture(BYTE ucScreen);
...
void OsdFuncClearOsdAndXOffset(BYTE ucRow, BYTE ucCol, BYTE ucXOffset, BYTE ucWidth, BYTE ucHeight)	
{
    WORD usOsdAddr = (WORD)ucRow * g_ucOsdWidth + ucCol + g_usFontSelectStart + ucXOffset;
    BYTE ucHeightCount = 0;
...
{
    //static BYTE cnt2 = 0;
    BYTE code colortable2[6][3] = {{0xff, 0x00, 0x00}, {0x00, 0xff, 0x00}, {0x00, 0x00, 0xff}, {0xff, 0xff, 0xff}, {0x7F, 0x7F, 0x7F}, {0x00, 0x00, 0x00}};
    //   if (UserCommonLGPanel_OFF_RS_OperationStatus() == _OFF_RS_OPERATION_HANDLER_STATUS_DISABLE)
    //
    SET_OSD_LCD_TEST_STATUS(_ON);
    ...
    ScalerDDomainBackgroundSetColor( colortable2[ucScreen][0], colortable2[ucScreen][1], colortable2[ucScreen][2]);
    ScalerDDomainBackgroundEnable( _ENABLE);

    //(++cnt2 > 5) ? (cnt2 = 0) : cnt2;
    //    }
    ScalerTimerActiveTimerEvent(SEC(10), _USER_TIMER_EVENT_LCD_TEST);
}
