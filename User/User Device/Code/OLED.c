...
bit _bPowerOnOffRS = _TRUE; //_FALSE; // POWER ON 시에 동작하는 OFF RS는 신호가 없으면 POWER SAVE에 빠집니다.
bit sbPowerSaveOffRS = _FALSE;
//--------------------------------------------------
...
BYTE AMOLED_ReadByte(BYTE B_RegAddr)	
{
	...
	/*
	if (ScalerMcuHwIICRead(_AMOLED_I2C_ADDR, 1, B_RegAddr, 1, Ba_Temp, _HW_IIC_PIN_AD28_AH27) == _FAIL)
	{
...
#if(_PCB_SYS_PANEL_IIC == _SW_IIC_PIN_GPIO)
	if(UserExternalDeviceIICRead(_AMOLED_I2C_ADDR, B_RegAddr, 1, 1, Ba_Temp, _IIC_PANEL, _PCB_SYS_PANEL_IIC) == _FAIL)
	{
        //return _FAIL;
	}
#elif(_PCB_SYS_PANEL_IIC != _NO_IIC_PIN)   // Else of #if(_PCB_SYS_PANEL_IIC == _SW_IIC_PIN_GPIO)
	if(UserExternalDeviceIICRead(_AMOLED_I2C_ADDR, B_RegAddr, 1, 1, Ba_Temp, 0x00, _PCB_SYS_PANEL_IIC) == _FAIL)
    {
    	//return _FAIL;
	}
#endif  // End of #if(_PCB_SYS_PANEL_IIC == _SW_IIC_PIN_GPIO)
...
void AMOLED_WriteByte(BYTE B_RegAddr, BYTE B_Data)	
{
	...
	Ba_Temp[0] = B_Data;
	/*
	if (ScalerMcuHwIICWrite(_AMOLED_I2C_ADDR, 1, B_RegAddr, 1, Ba_Temp, _HW_IIC_PIN_AD28_AH27) == _FAIL)
	{
...
#if(_PCB_SYS_PANEL_IIC == _SW_IIC_PIN_GPIO)
	if(UserExternalDeviceIICWrite(_AMOLED_I2C_ADDR, B_RegAddr, 1, 1, Ba_Temp, _IIC_PANEL, _PCB_SYS_PANEL_IIC, _WRITE_SIZE_8) == _FAIL)
    {
        //return _FAIL;
    }

#elif(_PCB_SYS_PANEL_IIC != _NO_IIC_PIN)   // Else of #if(_PCB_SYS_PANEL_IIC == _SW_IIC_PIN_GPIO)
	if(UserExternalDeviceIICWrite(_AMOLED_I2C_ADDR, B_RegAddr, 1, 1, Ba_Temp, 0x00, _PCB_SYS_PANEL_IIC, _WRITE_SIZE_8) == _FAIL)
    {
        //return _FAIL;
    }
#endif  // End of #if(_PCB_SYS_PANEL_IIC == _SW_IIC_PIN_GPIO)
