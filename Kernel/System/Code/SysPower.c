...
void SysPowerHandler(void)
{
		...
		//#if(_PANEL_IDX_NAME	 == _PANEL_LG_LW550AQD)
		//DebugMessageOsd("T:AC OFF 1", PCB_AC_DETECT());
		//DebugMessageOsd("T:AC OFF 1", POWER_ACOFF_DET);
		if (ucACOffDet != POWER_ACOFF_DET)//&&(bBACKLIGHTPOWER == _ON))
		{
			//DebugMessageTEST("T:POWER_ACOFF_DET", POWER_ACOFF_DET);
			//DebugMessageOsd("T:AC OFF 1", POWER_ACOFF_DET);
			if (POWER_ACOFF_DET == _OFF)
			{
				...
				//PCB_AC_DET_POWER(_LIGHT_CONTROL_OFF);
				// ScalerTimerActiveTimerEvent(SEC(0.5), _USER_TIMER_EVENT_AC_OFF_BLINK_TIMER);
	
				//DebugMessageOsd("T:AC OFF 1", POWER_ACOFF_DET);
				DebugMessageOsd("T:AC OFF 2", POWER_ACOFF_DET);
				// PCB_PANEL_POWER(_PANEL_CONTROL_OFF);
	
				//		ScalerTimerDelayXms(250);
				//		SET_TARGET_POWER_STATUS(_POWER_STATUS_OFF);
			}
			else DebugMessageOsd("T:AC ON", POWER_ACOFF_DET);
	
			ucACOffDet = POWER_ACOFF_DET;
		}
#endif
    ...
    switch(GET_TARGET_POWER_STATUS())
    {
        ...
        case _POWER_STATUS_OFF:
            ...
            switch(GET_POWER_STATUS())
            {
                ...
                case _POWER_STATUS_NORMAL:
                    ...
                    break;
#if (_ENABLE_MENU_OLED == _ON)
				case _POWER_STATUS_COMPENSATION_OFF_RS:
					if(SysPowerCheckFakeOff() == _FALSE)
					{
						CLR_POWER_SWITCH_TO_FAKE_OFF();
						SysPowerSwitch(_POWER_ACTION_NORMAL_TO_OFF);
		...
		//-----------------------------------------------------------------------------
			...
			case _POWER_STATUS_COMPENSATION_OFF_RS:
				CLR_POWER_SWITCH_TO_FAKE_SAVING();
				if(GET_POWER_STATUS() != GET_TARGET_POWER_STATUS())
				{
					 SET_POWER_STATUS(GET_TARGET_POWER_STATUS());
					 DebugMessageOsd("GET_TARGET_POWER_STATUS()", GET_TARGET_POWER_STATUS());
				}
		
				if ((GetWaitingToRunOFFRS() == _ON) && (bBACKLIGHTPOWER == _LIGHT_CONTROL_ON))
				{
					if (GET_OFF_RS_STATUS() == _OFF)
					{
						DebugMessageOsd("T:Tartget OFF : Excute_OLED_OFFRS", PIN_JB_OFF_RS_DONE);
						...
						Excute_OLED_OFFRS(_ON);
					}
					if ((PIN_JB_OFF_RS_DONE == _ON) && (GET_OFF_RS_STATUS() == _ON))
					{
						DebugMessageOsd("T:** PIN OFF_RS_DONE **", PIN_JB_OFF_RS_DONE);
						CancelWaitingToRunOFFRS();
						Excute_OLED_OFFRS(_OFF);
						ResetOffRSTime();
						ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_LED_BLINKING_TIMER);
						ScalerTimerCancelTimerEvent(_USER_TIMER_EVENT_OFFRS_END_TIMER);
						...
						if (_bPowerOnOffRS == _TRUE)
						{
							if(sbPowerSaveOffRS == _TRUE)
							{
							   ...
							   // PCB_D1_HOTPLUG(_D1_HOT_PLUG_HIGH);
								...
								DebugMessageOsd("_POWER_STATUS_SAVING", _POWER_STATUS_SAVING);
						...
						else
						{
							...
							PCB_D1_HOTPLUG(_D3_HOT_PLUG_LOW);
							ScalerTimerDelayXms(3000);
							DebugMessageOsd("T:_bPowerOnOffRS == _false", PIN_JB_OFF_RS_DONE);
							//SysPowerSetTargetPowerStatus(_POWER_STATUS_NORMAL);
			...
			case _POWER_STATUS_COMPENSATION_JB_TO_ON:
				CLR_POWER_SWITCH_TO_FAKE_SAVING();
				if ((GetWaitingToRunJB() == _ON) && (bBACKLIGHTPOWER == _LIGHT_CONTROL_ON))
				{
					if (GET_JB_COMP_STATUS() == _OFF)
					{
						DebugMessageOsd("T:Tartget OFF : OLED_JB_Excute", PIN_JB_OFF_RS_DONE);
						ScalerTimerActiveTimerEvent(SEC(0.5), _USER_TIMER_EVENT_LED_BLINKING_TIMER);
						ScalerTimerActiveTimerEvent(SEC(60), _USER_TIMER_EVENT_JB_END_TIMER); //200916 by hoon for 65 UHD Time Extension
						UserInterfaceKeyPadLedControl(_LED_TYPE_FLASH);
						Excute_OLED_JB(_ON);
					}
		
					if ((PIN_JB_OFF_RS_DONE == _ON) && (GET_JB_COMP_STATUS() == _ON))
					{
						DebugMessageOsd("T:** PIN JB DONE **", PIN_JB_OFF_RS_DONE);
						CancelWaitingToRunJB();
						Excute_OLED_JB(_OFF);
						ResetJBTime();
						RTDNVRamSaveOSDData();
						RTDNVRamSaveOsdServiceData();
	...
	/*
						...
						if (SysRegionGetDisplayState(_DISPLAY_RGN_1P) == _DISPLAY_STATE_NO_SIGNAL)
							ScalerTimerActiveTimerEvent(SEC(2), _USER_TIMER_EVENT_OSD_SHOW_NO_SIGNAL);
	*/
					}
				}
...
#if (_ENABLE_MENU_OLED == _ON)
	if((GET_TARGET_POWER_STATUS() != _POWER_STATUS_COMPENSATION_OFF_RS) && (GET_TARGET_POWER_STATUS() != _POWER_STATUS_COMPENSATION_JB_TO_ON))
#endif
	{
	// Clear power action to avoid repeat calls in next circle.
		SET_TARGET_POWER_STATUS(_POWER_STATUS_NONE);
