...
//--------------------------------------------------
#define _DISP_CALIB_FW_VERSION      0x02
...
//****************************************************************************************//
        ...
        else if(ucCalibrationStep == _DDCCI_DISP_CALIB_SET_DICOM)
        {
            // Set DICOM
            ScalerColorOutputGammaEnable(_FUNCTION_OFF);
            ScalerColorOutputGammaAdjust(UserCommonAdjustDICOMAddr(_DICOM1_TYPE), UserCommonAdjustDICOMBank(_DICOM1_TYPE));
            ScalerColorOutputGammaEnable(_FUNCTION_ON);
        ...
        {
            ScalerDDomainBackgroundEnable(_ENABLE);
            ucR = g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_VERIFY_RED];
            ucG = g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_VERIFY_GREEN];
            ucB = g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_VERIFY_BLUE];

            ScalerDDomainBackgroundSetColor(ucR, ucG, ucB);
        }
        ...
        if(ucCalibrationStep == _DDCCI_DISP_CALIB_PANEL_COLOR_MATCHING_DATA)
        {
            ...
            {
                for(ucCountPage = _OGC_FLASH_PAGE; ucCountPage <= _OGC_FLASH_PAGE + 2; ucCountPage++)
                {
...
BYTE UserCommonDdcciDispCalibCalCheckSum(BYTE ucCheckType)
{
    ...
    switch(ucCheckType)
    {
        ...
        case _COLOR_MATRIX_TYPE2:
            ...
            for(usI = _OCC_EBU_ADDRESS; usI < (_OCC_DISPLAYP3_ADDRESS + _OCC_COLORMATRIX_TABLE_SIZE); usI++)
            {
                UserCommonFlashRead(_OGC_FLASH_BANK, ((((WORD)_OGC_FLASH_PAGE) << 12) + usI), 1, pData + 2);
                pData[0] += pData[2];
            }
...
void UserCommonDdcciDispCalibReplyResult(BYTE ucType)
{
    ...
    switch(ucType)
    {
        case _DISP_CALIB_START:
            ...
            g_pucDdcciTxBuf[_DDCCI_SOURCE] = _DDCCI_DEST_ADDRESS;
            g_pucDdcciTxBuf[_DDCCI_LENGTH] = _DDCCI_CONTROL_STATUS_FLAG | 0x04;
            g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_OGCOCCDICOM_SUPPORT] = (_DCIP3_NO_COMPRESSION_SUPPORT << 3) | (_OGC_DICOM_SUPPORT << 2) | (_OCC_SUPPORT << 1) | (_OGC_SUPPORT);
        ...
        case _DISP_CALIB_USER_DEFINE:
            ...
            if(g_pucDdcciRxBuf[_DDCCI_DISP_CALIB_USER_DEFINE_TYPE] == _DDCCI_DISP_CALIB_USER_DDCCI_LENGTH)
            {
                ...
                g_pucDdcciTxBuf[_DDCCI_DISP_CALIB_STRING_2] = (_DISP_CALIB_FW_VERSION & 0xFF);
