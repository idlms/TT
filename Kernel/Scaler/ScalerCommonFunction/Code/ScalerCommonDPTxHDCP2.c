...
void ScalerDpTxHdcp2Handler(void)
{
    ...
    switch(GET_DP_TX_HDCP2_AUTH_STATE())
    {
        ...
        case _HDCP_2_2_TX_STATE_VERIFY_SIGNATURE:
            ...
            if(GET_DP_TX_HDCP2_LOAD_KEY_FINISH() == _TRUE)
            {
...
bit ScalerDpTxHdcp2SessionKeyExchangeProc(void)
{
    ...
    memcpy(g_pucDpTxHdcp2Riv, pucdRiv, sizeof(g_pucDpTxHdcp2Riv));
...
bit ScalerDpTxHdcp2VerifyVprime(void)
{
    ...
    // Read Rxinfo
    pucRxInfo[0] = ((GET_DP_TX_HDCP2_RXINFO_DEPTH() << 1) | (GET_DP_TX_HDCP2_RXINFO_DEVICE_COUNT() >> 4));
    pucRxInfo[1] = ((GET_DP_TX_HDCP2_RXINFO_DEVICE_COUNT() << 4) |
                    ((BYTE)GET_DP_TX_HDCP2_RXINFO_DEVICE_COUNT_EXCEED() << 3) |
                    ((BYTE)GET_DP_TX_HDCP2_RXINFO_DEPTH_EXCEED() << 2) |
                    ((BYTE)GET_DP_TX_HDCP2_RXINFO_HDCP20_REPEATER_DOWNSTREAM() << 1) |
                    (GET_DP_TX_HDCP2_RXINFO_HDCP1_DOWNSTREAM()));
...
bit ScalerDpTxHdcp2ContentStreamManagement(void)
{
    ...
    if(GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST)
    {
        ...
        for(enumStSourceTemp = _DP_ST_SOURCE3; enumStSourceTemp <= _DP_ST_SOURCE5; enumStSourceTemp++)
        {
            if(ScalerDpRxMstStreamMuxCheck(enumStSourceTemp) != 0)
            {
                // Get Stream Source X Status
                ucStreamId = ScalerDpRxMstStreamMuxCheck(enumStSourceTemp) - 1;

                // Get Stream IDx in STx
                pucStreamIDType[usK * 2] = g_pucDpStreamToIdMapping[ucStreamId];

                if(ScalerSyncDpGetHdcp2ModeStatus() == _TRUE)
                {
                    // Get Type of Stream IDx
                    pucStreamIDType[(usK * 2) + 1] = GET_DP_TX_HDCP2_RX_STX_TYPE(ucStreamId);
                }
                else
                {
                    // Get Type of Stream IDx
                ...
                ScalerSetBit((PBB_6C_DP_STREAM_1_PBN + usK), ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), GET_DP_TX_STX_TIMESLOT_LENGTH(enumStSourceTemp));

                DebugMessageHDCP2("CSM - STx Source", ucStreamId);
                DebugMessageHDCP2("CSM - STx Stream ID Type", pucStreamIDType[usK * 2]);
                DebugMessageHDCP2("CSM - TimeSlot Length", GET_DP_TX_STX_TIMESLOT_LENGTH(enumStSourceTemp));
                    ...
                    if(GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST)
                    {
                        for(enumStSourceTemp = _DP_ST_SOURCE3; enumStSourceTemp <= _DP_ST_SOURCE5; enumStSourceTemp++)
                        {
                            if(ScalerDpRxMstStreamMuxCheck(enumStSourceTemp) != 0)
                            {
                                ...
                                // Set Length of STx Stream to Be Ready For Encryption
                                ScalerSetBit(PBB_6C_DP_STREAM_1_PBN, ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), GET_DP_TX_STX_TIMESLOT_LENGTH(enumStSourceTemp));
                ...
                if(GET_DP_TX_STREAM_SOURCE() == _DP_TX_SOURCE_MST)
                {
                    ...
                    for(enumStSourceTemp = _DP_ST_SOURCE3; enumStSourceTemp <= _DP_ST_SOURCE5; enumStSourceTemp++)
                    {
                        if(ScalerDpRxMstStreamMuxCheck(enumStSourceTemp) != 0)
                        {
                            ...
                            // Set Length of STx Stream to Be Ready For Encryption
                            ScalerSetBit((PBB_6C_DP_STREAM_1_PBN + usKValue), ~(_BIT5 | _BIT4 | _BIT3 | _BIT2 | _BIT1 | _BIT0), GET_DP_TX_STX_TIMESLOT_LENGTH(enumStSourceTemp));
