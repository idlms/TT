...
//--------------------------------------------------
BYTE ScalerHdcp2RsaAvailable_EXINT0(void) using 1
{
    // (M2PLL Power ON) && (GDI Clock is M2PLL)
    if((ScalerGetBit_EXINT(P1_E5_M2PLL_WD, _BIT0) == 0x00) && (ScalerGetBit_EXINT(P80_06_GDI_CLK_DIVIDER, _BIT4) == _BIT4))
    {
        return _TRUE;
    }
...
// Output Value : None
//--------------------------------------------------
void ScalerHdcp2RxRsaCal_EXINT0(BYTE *pucEkpubKmArray) using 1
{
...
// Output Value : _TRUE or _FALSE
//--------------------------------------------------
BYTE ScalerHdcp2RsaAvailable_WDINT(void) using 3
{
    // (M2PLL Power ON) && (GDI Clock is M2PLL)
    if((ScalerGetBit_EXINT(P1_E5_M2PLL_WD, _BIT0) == 0x00) && (ScalerGetBit_EXINT(P80_06_GDI_CLK_DIVIDER, _BIT4) == _BIT4))
    {
...
// Output Value : None
//--------------------------------------------------
BYTE ScalerHdcp2RxRsaOaepCalculate_WDINT(BYTE *pucKmArray) using 3
{
    ...
    if(ScalerHdcp2RxSha256Calculate_WDINT(&pucHdcp2RsaOaepInput[33], 0) == _TRUE)
    {
        ...
        if(ScalerHdcp2RxSha256Calculate_WDINT(g_pucHdcp2SHA256Input, 1) == _TRUE)
        {
            ...
            ScalerHdcp2RxSha256Calculate_WDINT(g_pucHdcp2SHA256Input, 0);
            ...
            // DB maskDB XOR DBmask
            for(pData_EXINT[0] = 0; pData_EXINT[0] < 31; pData_EXINT[0]++)
            {
...
// Output Value : None
//--------------------------------------------------
BYTE ScalerHdcp2RxSha256Calculate_WDINT(BYTE *pucInputArray, BYTE ucCalCount) using 3
{
    BYTE ucLoopIndex = 0;
...
// Output Value : None
//--------------------------------------------------
BYTE ScalerHdcp2HPrimeCalculate_WDINT(BYTE *pucHdcp2dKey0, BYTE *pucHdcp2dKey1, BYTE *pucHdcp2Rtx, BYTE *pucHdcp2TxCaps, BYTE *pucHdcp2RxCaps) using 3
{
    // H = HMAC-SHA256(rtx || RxCaps || TxCaps, kd)
    // HMAC(text) = HMAC(K, text) = SHA256((K0 XOR opad) || SHA256((KO XOR ipad) || text))
    // Calculate SHA256((KO XOR ipad) || text)
...
// Output Value : None
//--------------------------------------------------
BYTE ScalerHdcp2RxHmacSha256Calculate_WDINT(BYTE *pucInputArray1, BYTE *pucInputArray2) using 3
{
    for(pData_EXINT[1] = 0; pData_EXINT[1] < 64; pData_EXINT[1]++)
    {
        // K0 XOR ipad = 512 bit
...
// Output Value : None
//--------------------------------------------------
void ScalerHdcp2RxRsaCal_WDINT(BYTE *pucEkpubKmArray) using 3
{
    // Enable HDCP 2.2 RSA Module
    ScalerSetBit_EXINT(P63_10_HDCP_RSA_CTRL0, ~(_BIT7 | _BIT6 | _BIT5), (_BIT7 | _BIT5));
