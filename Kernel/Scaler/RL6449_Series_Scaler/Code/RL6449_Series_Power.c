...
BYTE ScalerPowerIsolation(EnumPowerAction enumSwitch)
{
    ...
    case _POWER_ACTION_AC_ON_INITIAL:
        ...
        if (enumSwitch != _POWER_ACTION_AC_ON_INITIAL) //  No Power Cut when SMPS AC on
...
void ScalerPowerGroupAPowerCut(BYTE ucOn)
{
    if (ucOn == _ON)
    {
        ...
        PCB_REGULATOR_5V_OUT(_REGULATOR_5V_OFF); // SMPS Power Off
    }
    else
    {
        ...
        {
            PCB_REGULATOR_5V_OUT(_REGULATOR_5V_ON); // SMPS Power On
